...
{
	static void* control_function = GetProcAddress(LoadLibrary("win32u.dll"), "NtDxgkVailPromoteCompositionSurface");
...
namespace driver
{
	...
	static bool open_memory_handles()
	{
		...
		memory_read = OpenFileMapping(FILE_MAP_READ, FALSE, "Global\\champainvf");
		if (memory_read == INVALID_HANDLE_VALUE || !memory_read)
			return false;

		memory_write = OpenFileMapping(FILE_MAP_WRITE, FALSE, "Global\\champainvf");
		if (memory_write == INVALID_HANDLE_VALUE || !memory_write)
			return false;

		memory_esp_write = OpenFileMapping(FILE_MAP_WRITE, FALSE, "Global\\champainvf58");
